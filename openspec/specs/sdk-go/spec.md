...
Defines the observable client behavior third-party integrators depend on when calling hush-hush from Go: how a client is constructed, what typed operations it exposes, and how it surfaces errors, retries, and pagination.
...
- **WHEN** a caller invokes the create method with a valid typed request struct
- **THEN** the SDK sends the corresponding HTTP request and returns a typed response struct on success
...
The SDK SHALL expose a typed method for querying the audit log with optional `object_id`, `caller`, `from`, and `to` filters, returning the full matching result set as a single typed slice. hush-hush's `/audit-log` endpoint has no pagination parameters, so the SDK SHALL NOT invent a cursor or iterator over it.
...
The SDK SHALL map any non-2xx HTTP response to a typed error carrying the HTTP status code, the parsed error response body, and a request ID when the response provides one — never a bare string or the raw HTTP response.

#### Scenario: Server returns a 4xx validation error
- **WHEN** the server responds with a non-2xx, non-retryable status
- **THEN** the SDK returns a typed error containing that status and the parsed error body, without retrying
...
- **THEN** the SDK returns the typed error immediately without retrying
