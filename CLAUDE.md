...
- **`internal/genclient/client.gen.go` is generated by `oapi-codegen`; never hand-edit it.** It lives in its own internal package specifically so its exported names (`Client`, `NewClient`, `ClientOption`, ...) don't collide with this package's own hand-written `Client`/`NewClient` — see `types.go` for the type aliases that re-export its models.
  `codegen.yml`'s drift check fails CI if it doesn't match what regenerating
  from the pinned spec submodule produces.
- **`api/` is a git submodule pinned to a hush-hush commit, not this repo's
  own spec.** Clone with `--recurse-submodules`. A spec change belongs in
  hush-hush; this repo picks it up on the next scheduled regeneration.
